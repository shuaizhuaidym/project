...
BROWSER.ie = window.ActiveXObject && USERAGENT.indexOf('msie') != -1
...
BROWSER.chrome = window.MessageEvent && !document.getBoxObjectFor
		&& USERAGENT.indexOf('chrome') != -1
		&& USERAGENT.substr(USERAGENT.indexOf('chrome') + 7, 10);
BROWSER.opera = window.opera && opera.version();
BROWSER.safari = window.openDatabase && USERAGENT.indexOf('safari') != -1
		&& USERAGENT.substr(USERAGENT.indexOf('safari') + 7, 8);
BROWSER.other = !BROWSER.ie && !BROWSER.firefox && !BROWSER.chrome
	...
	if (BROWSER.ie) {
		...
		var msg = '<div style="text-decoration:underline;">点此复制到剪贴板</div>'
				+ AC_FL_RunContent('id', 'clipboardswf', 'name',
						'clipboardswf', 'devicefont', 'false', 'width', '120',
						'height', '40', 'src', 'images/common/clipboard.swf',
	...
	for (var i = 0; i < obj.elements.length; i++) {
		...
		if (el.name != ''
				&& (el.tagName == 'TEXTAREA' || el.tagName == 'INPUT'
						&& (el.type == 'text' || el.type == 'checkbox' || el.type == 'radio'))
...
function toggle_collapse(objname, noimg, complex, lang) {
	...
	if (complex) {
		var objc = $(objname + '_c');
		if (objc) {
			objc.className = objc.className == 'c_header' ? 'c_header closenode'
					: 'c_header';
		}
	}
}
...
function loadcss(cssname) {
	if (!CSSLOADED[cssname]) {
		...
		css.type = 'text/css';
		css.rel = 'stylesheet';
		css.href = 'forumdata/cache/style_' + STYLEID + '_' + cssname + '.css?'
				+ VERHASH;
...
function setMenuPosition(showid, menuid, pos) {
	...
	if (in_array(direction, [ 1, 4 ]) && ml < 0) {
		...
		if (in_array(basePoint, [ 1, 4 ]))
			ml += sw;
	} else if (ml + mw > document.documentElement.scrollLeft
	...
	if (in_array(direction, [ 1, 2 ]) && mt < 0) {
		...
		if (in_array(basePoint, [ 1, 2 ]))
			mt += sh;
	} else if (mt + mh > document.documentElement.scrollTop
	...
	if (direction == 0 || menuObj.scrolly) {
		if (BROWSER.ie && BROWSER.ie < 7) {
			if (direction == 0)
				mt += Math.max(document.documentElement.scrollTop,
...
function showPrompt(ctrlid, evt, msg, timeout, negligible) {
	...
	if (!$(menuid)) {
		...
		if (ctrlid) {
			msg = '<div id="' + ctrlid
					+ '_prompt" class="promptcontent"><ul><li>' + msg
					+ '</li></ul></div>';
		} else {
			msg = negligible ? msg : '<span style="font-style: normal;">' + msg
					+ '</span>';
			msg = '<table cellspacing="0" cellpadding="0" class="popupcredit"><tr><td class="pc_l">&nbsp;</td><td class="pc_c"><div class="pc_inner">'
					+ msg
					+ (negligible ? '<a class="pc_btn" href="javascript:;" onclick="display(\'ntcwin\');setcookie(\'discuz_creditnoticedisable\', 1, 31536000);" title="不要再提示我"><img src="'
							+ IMGDIR
...
function showCreditPrompt() {
	...
	if (notice.length < 2 || notice[9] != discuz_uid
			|| getcookie('discuz_creditnoticedisable')) {
		return;
	}
	...
	for (i = 1; i <= 8; i++) {
		if (notice[i] != 0 && creditinfo[i]) {
			s += '<span>' + creditinfo[i][0]
					+ (notice[i] > 0 ? '<em>+' : '<em class="desc">')
...
function showDialog(msg, mode, t, func, cover) {
	...
	var s = '<table cellpadding="0" cellspacing="0" class="fwin"><tr><td class="t_l"></td><td class="t_c"></td><td class="t_r"></td></tr><tr><td class="m_l"></td><td class="m_c"><div class="fcontent'
			+ (mode == 'info' ? '' : ' alert_win')
			+ '"><h3 class="float_ctrl"><em>';
	s += t ? t : '提示信息';
	s += '</em><span><a href="javascript:;" class="float_close" onclick="hideMenu(\''
			+ menuid + '\', \'dialog\')" title="关闭">关闭</a></span></h3>';
	if (mode == 'info') {
		s += msg ? msg : '';
	} else {
		s += '<hr class="shadowline" />';
		s += '<div class="postbox"><div class="'
				+ (mode == 'alert' ? 'alert_error' : 'alert_info') + '"><p>'
				+ msg + '</p></div>';
		s += '<div class="alert_btn"><input type="button" id="fwin_dialog_submit" value="&nbsp;确定&nbsp;" />';
		s += mode == 'confirm' ? '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" onclick="hideMenu(\''
				+ menuid + '\', \'dialog\')" value="&nbsp;取消&nbsp;" />'
				: '';
		s += '</div></div>';
	}
...
function showWindow(k, url, mode, cache) {
	...
	var show = function() {
		...
		showMenu({
			...
			'drag' : typeof drag == 'undefined' ? '' : drag,
			'cache' : cache
		});
	};
	if (!menuObj) {
		...
		menuObj.style.display = 'none';
		$('append_parent').appendChild(menuObj);
		menuObj.innerHTML = '<table cellpadding="0" cellspacing="0" class="fwin"><tr><td class="t_l"></td><td class="t_c"></td><td class="t_r"></td></tr><tr><td class="m_l"></td><td class="m_c" id="fwin_content_'
				+ k
				+ '">'
				+ '</td><td class="m_r"></td></tr><tr><td class="b_l"></td><td class="b_c"></td><td class="b_r"></td></tr></table>';
		fetchContent();
	} else if ((mode == 'get' && url != menuObj.url)
		...
		if (aj.XMLHttpRequest.readyState == 4
			...
			if (aj.recvType == 'HTML') {
				...
				if (aj.XMLHttpRequest.responseXML.lastChild) {
					...
					if (AJAX['debug']) {
						var error = mb_cutstr(
								aj.XMLHttpRequest.responseText.replace(
										/\r?\n/g, '\\n').replace(/"/g, '\\\"'),
								200);
						aj
								.resultHandle(
										'<root>ajaxerror<script type="text/javascript" reload="1">showDialog(\'Ajax Error: \\n'
												+ error
												+ '\');</script></root>', aj);
					}
				}
...
function ajaxpost(formid, showid, waitid, showidclass, submitbtn, recall) {
	...
	var handleResult = function() {
		...
		try {
			...
			if (AJAX['debug']) {
				var error = mb_cutstr(
						$(ajaxframeid).contentWindow.document.body.innerText
								.replace(/\r?\n/g, '\\n').replace(/"/g, '\\\"'),
						200);
...
function ajaxmenu(ctrlObj, timeout, cache, duration, pos, recall) {
	...
	if (menu) {
		...
		menu.innerHTML = '<div class="popupmenu_option" id="' + menuid
				+ '_content"></div>';
		$('append_parent').appendChild(menu);
		ajaxget(!isUndefined(ctrlObj.href) ? ctrlObj.href
				: ctrlObj.attributes['href'].value, menuid + '_content',
				'ajaxwaitid', '', '', func);
	}
}
...
function simulateSelect(selectId) {
	...
	selectObj.outerHTML += '<a href="javascript:;" hidefocus="true" id="'
			+ selectId + '_ctrl" tabindex="1">' + defaultopt + '</a>';
	...
	$(selectId + '_ctrl').onclick = function(e) {
		$(selectId + '_ctrl_menu').style.width = (BROWSER.ie ? Math.max(
				$(selectId + '_ctrl').clientWidth, parseInt($(selectId
						+ '_ctrl').currentStyle.width))
				: $(selectId + '_ctrl').clientWidth)
				+ 'px';
		showMenu({
...
function detectCapsLock(e, obj) {
	...
	obj.className = (valueCapsLock >= 65 && valueCapsLock <= 90 && !valueShift || valueCapsLock >= 97
			&& valueCapsLock <= 122 && valueShift) ? 'capslock txt' : 'txt';
	if (BROWSER.ie) {
		...
		for (var i = 0; i < 40; i++) {
			str += '<input type="button" style="background-color: '
					...
					+ (typeof wysiwyg == 'undefined' ? 'seditor_insertunit(\''
							+ k + '\', \'[color=' + coloroptions[i]
							+ ']\', \'[/color]\')' : (ctrlid == editorid
							+ '_cmd_table_param_4' ? '$(\'' + ctrlid
							+ '\').value=\'' + coloroptions[i]
							+ '\';hideMenu(2)' : 'discuzcode(\'forecolor\', \''
...
function smilies_onload(id, smcols, seditorkey) {
	...
	if (typeof smilies_type == 'object') {
		...
		for (i in smilies_type) {
			if (smilies_type[i][0]) {
				smiliestype += '<li><a href="javascript:;" hidefocus="true" '
						+ (CURRENTSTYPE == i ? 'class="current"' : '')
						+ ' id="'
						+ seditorkey
						+ 'stype_'
						+ i
						+ '" onclick="smilies_switch(\''
						+ id
						+ '\', \''
						+ smcols
						+ '\', '
						...
						+ '\');if(CURRENTSTYPE) {$(\''
						+ seditorkey
						+ 'stype_\'+CURRENTSTYPE).className=\'\';}this.className=\'current\';CURRENTSTYPE='
						+ i + ';doane(event);">' + smilies_type[i][0]
						+ '</a></li>';
			}
		}
		smiliestype += '</ul></div>';
		$(id).innerHTML = smiliestype + '<div id="' + id
				+ '_data"></div><div class="smilieslist_page" id="' + id
				+ '_page"></div>';
...
function smilies_switch(id, smcols, type, page, seditorkey) {
	...
	smiliesdata = '<table id="' + id
			+ '_table" cellpadding="0" cellspacing="0"><tr>';
	j = k = 0;
	img = [];
	for (i in smilies_array[type][page]) {
		...
		img[k] = new Image();
		img[k].src = smilieimg;
		smiliesdata += s && s[0] ? '<td onmouseover="smilies_preview(\''
				+ seditorkey
				+ '\', \''
				+ id
				+ '\', this, '
				+ s[5]
				+ ')" onclick="'
				+ (typeof wysiwyg != 'undefined' ? 'insertSmiley(' + s[0] + ')'
						: 'seditor_insertunit(\'' + seditorkey + '\', \''
								+ s[1].replace(/'/, '\\\'') + '\')') + '" id="'
				+ seditorkey + 'smilie_' + s[0] + '_td"><img id="smilie_'
				+ s[0] + '" width="' + s[3] + '" height="' + s[4] + '" src="'
				+ smilieimg + '" alt="' + s[1] + '" />' : '<td>';
		j++;
		k++;
	}
	smiliesdata += '</table>';
	...
	if (smilies_array[type].length > 2) {
		prevpage = ((prevpage = parseInt(page) - 1) < 1) ? smilies_array[type].length - 1
				: prevpage;
		nextpage = ((nextpage = parseInt(page) + 1) == smilies_array[type].length) ? 1
				: nextpage;
		smiliespage = '<div class="pags_act"><a href="javascript:;" onclick="smilies_switch(\''
				+ id
				+ '\', \''
				+ smcols
				+ '\', '
				...
				+ seditorkey
				+ '\');doane(event);">上页</a>'
				+ '<a href="javascript:;" onclick="smilies_switch(\''
				+ id
				+ '\', \''
				+ smcols
				+ '\', '
				...
				+ ', \''
				+ seditorkey
				+ '\');doane(event);">下页</a></div>'
				+ page
				+ '/'
				+ (smilies_array[type].length - 1);
	}
...
function smilies_preview(seditorkey, id, obj, w) {
	...
	pos = spos['left'] >= mpos['left'] + $(id + '_data').offsetWidth / 2 ? '13'
			: '24';
	showMenu({
		'ctrlid' : obj.id,
		'showid' : id + '_data',
...
function seditor_insertunit(key, text, textend, moveend) {
	...
	if (!isUndefined($(key + 'message').selectionStart)) {
		...
		if (textend != '') {
			text = text
					+ $(key + 'message').value.substring(
							$(key + 'message').selectionStart, $(key
									+ 'message').selectionEnd) + textend;
		}
		$(key + 'message').value = $(key + 'message').value.substr(0, $(key
				+ 'message').selectionStart)
				+ text
				+ $(key + 'message').value
						.substr($(key + 'message').selectionEnd);
		if (!moveend) {
...
function parseurl(str, mode, parsecode) {
	...
	str = str
			.replace(
					...
					mode == 'html' ? '$1<img src="$2" border="0">'
							: '$1[img]$2[/img]');
	str = str
			.replace(
...
function pmchecknew() {
	ajaxget('pm.php?checknewpm=' + Math.random(), 'myprompt_check',
			'ajaxwaitid');
}
...
if (typeof IN_ADMINCP == 'undefined') {
	...
	if (creditnotice != '' && getcookie('discuz_creditnotice')
			&& !getcookie('discuz_creditnoticedisable')) {
